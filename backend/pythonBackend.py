...
def jsonCodeGenerator(jsonObj):
        ...
        print(data)
        print("=============== RAMAN =====================")
        circCreator, numOfInputLines = CreateCircuit(jsonObj)
        print("======== Going to call APPLY ==============")
        output=circCreator.Apply()
        print("############ OUTPUT #############")
        print(output)
        print("#################################")
        #return data
        return OutputToJson(output,numOfInputLines)   
        ...
        '''
def OutputToJson(output,numOfInputLines):
        yList=output[:,0]
        yStringList=[abs(item)[0]**2 for item in yList]
        xList=[bin(val) for val in range(2**numOfInputLines)]
...
def Apply():
        return circCreator.Apply()    

#Python code to call QuTiP function
def CreateCircuit(gateJson):
        numOfInputLines=FindNumOfInputLines(gateJson)
